...
void Robot::save2map(std::vector<std::vector<double>> &pos,
  ...
  /*
  param this->point_cloud_: a hashmap containing point cloud entries (keys) and
  corresponding configurations (values) param pos: the end positions computed
  from forward_kinematics param configs: corresponding configurations of the
  robotic arm
  */
